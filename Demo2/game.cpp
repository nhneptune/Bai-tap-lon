...
void Game::gameStart(int argc, char* argv[]) {
    //Menu
    SDL_Texture* youWin = IMG_LoadTexture(renderer, "youWin.png");
    //Text
    TextBox scoreText("0", "font.ttf", 32, 255, 255, 255, 255, 60, 420);
    // clouds
...
//        }
    ...
    getFishSound = Mix_LoadWAV("getFish.wav");
    bgMusic = Mix_LoadMUS("backgroundMusic.mp3");
    //Game loop
    SDL_Event event;
    while (running) {
        //Play music
...
//            fish->moveFish(1);
            ...
            if (fishes[i].checkCollision(hook)) {
                ...
                hook.catchFish();
                fishes[i].gotHooked(); 
                Mix_PlayChannel(-1, getFishSound, 0);
                score += 100;
